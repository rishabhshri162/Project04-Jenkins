...
/**
 * Model class for managing Patient entity. Provides methods for add, update,
...
public class PatientModel {
	...
	public void update(PatientBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("UPDATE st_patient SET name=?, dateofvisit=?, mobileNo=?, disease=?, "
	...
	public PatientBean findByName(String name) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM st_patient WHERE name=?");
