...
@WebServlet(name = "HospitalAppointmentListCtl", urlPatterns = { "/ctl/HospitalAppointmentListCtl" })
public class HospitalAppointmentListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<HospitalAppointmentBean> list = model.search(bean, pageNo, pageSize);
			List<HospitalAppointmentBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					HospitalAppointmentBean deleteBean = new HospitalAppointmentBean();
					...
					ServletUtility.setSuccessMessage("Appointment deleted successfully", request);
	...
	protected String getView() {
		return ORSView.HOSPITAL_APPOINTMENT_LIST_VIEW;
	}
}