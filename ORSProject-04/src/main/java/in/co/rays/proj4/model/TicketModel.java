...
public class TicketModel {
	...
	public long add(TicketBean bean) {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(sd.toString());
			pstmt.setLong(1, pk);
	...
	public void delete(TicketBean bean) {
		Connection conn = null;
	...
	public void update(TicketBean bean) {
		...
		try {
			...
			StringBuffer sb = new StringBuffer(
					"UPDATE ticket_booking SET user_id=?, booking_date=?, seat_no=?, quantity=?, amount=?, payment_mode=?, booking_status=?, created_by=?, modified_by=?, created_datetime=?, modified_datetime=? WHERE id= ?");
	...
	public TicketBean findByPk(long pk) {
