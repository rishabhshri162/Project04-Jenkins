...
public class ClientModel {
	...
	public long add(ClientBean bean) throws ApplicationException, DuplicateRecordException {
		...
		ClientBean existBean = findByEmail(bean.getEmail());
		if (existBean != null) {
			throw new DuplicateRecordException("client already exists");
		}
	...
	public void update(ClientBean bean) throws ApplicationException, DuplicateRecordException {
		...
		ClientBean existBean = findByPk(bean.getId());
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("update st_client set client_name=?, email=?, phone=?, "
	...
	public ClientBean findByEmail(String email) throws ApplicationException {
		...
		try {
			...
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("select * from st_client where email=?");
			...
			throw new ApplicationException("Exception in getting Client by login");
		} finally {
			JDBCDataSource.closeConnection(conn);
		}
